...
try {
    ...
    $lk->execute([$course['id']]);
    $existingCategoryIds = array_map('intval', array_column($lk->fetchAll(PDO::FETCH_ASSOC), 'category_id'));
} catch (Exception $e) {
    $existingCategoryIds = [];
}
if (!$existingCategoryIds && !empty($course['category_id'])) { 
    $existingCategoryIds = [(int)$course['category_id']]; 
}

// Load existing media URLs from medias table
$existingDemoAudio = '';
$existingFileUrl = '';
$existingOriginalSource = '';
try {
    ...
    foreach ($medias as $media) {
        ...
        $val = $media['value'] ?? '';
        if ($ft === 'wav' || $ft === 'audio') {
            $existingDemoAudio = $val;
        } elseif ($ft === 'zip') {
            $existingFileUrl = $val;
        } elseif ($ft === 'pdf') {
            $existingOriginalSource = $val;
        }
    }
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $thumbnail_path = $course['cover_image'] ?? '';
    ...
    if (!$error) {
        ...
        if ($visibility === 'scheduled' && $publish_date) {
            $created_at = date('Y-m-d H:i:s', strtotime($publish_date));
        ...
        $stmt = $pdo->prepare("
            UPDATE courses SET
                ...
                price = ?, currency = 'TZS', cover_image = ?, promo_video = ?,
                visibility = ?, featured = ?, category_id = ?, level = ?, duration = ?, language = ?
            WHERE id = ?
        ");
        ...
        if ($stmt->execute([
            ...
            try {
                ...
                $del = $pdo->prepare('DELETE FROM course_category_links WHERE course_id = ?');
                $del->execute([$course['id']]);
                if (!empty($category_ids)) {
                    $ins = $pdo->prepare('INSERT IGNORE INTO course_category_links (course_id, category_id) VALUES (?, ?)');
                    foreach ($category_ids as $cid) {
                        if ($cid) {
                            $ins->execute([$course['id'], (int)$cid]);
                        }
                    }
                }
                ...
                $delm = $pdo->prepare('DELETE FROM medias WHERE courses_id = ? AND type = "url"');
                    $delm->execute([$course['id']]);
                
                // Insert new entries
                    if ($demo_audio_url !== '') {
                    $insm = $pdo->prepare('INSERT INTO medias (courses_id, file_type, value, type) VALUES (?, "wav", ?, "url")');
                        $insm->execute([$course['id'], $demo_audio_url]);
                    }
                if ($file_url !== '') {
                    $insm = $pdo->prepare('INSERT INTO medias (courses_id, file_type, value, type) VALUES (?, "zip", ?, "url")');
                    $insm->execute([$course['id'], $file_url]);
                }
                if ($original_source !== '') {
                    $insm = $pdo->prepare('INSERT INTO medias (courses_id, file_type, value, type) VALUES (?, "pdf", ?, "url")');
                    $insm->execute([$course['id'], $original_source]);
                }
                
                // Update Course Curriculum (Modules and Lessons)
                // Delete existing modules and lessons (cascade will handle lessons)
                $pdo->prepare("DELETE FROM modules WHERE course_id = ?")->execute([$course['id']]);
                ...
                if (isset($_POST['modules']) && is_array($_POST['modules'])) {
                    ...
                    $pdo->prepare("
                        ...
                        WHERE id = ?
                    ")->execute([$course['id'], $course['id'], $course['id']]);
                }
            ...
            $error = "Failed to update course in the database.";
        }
    }
}

$title = 'Edit Course - AK23 App';
    ...
    <form method="POST" enctype="multipart/form-data" id="courseForm">
        ...
        <div class="col-12">
                                ...
                                <textarea name="short_description" id="short_description" class="form-control rte" 
                                          rows="4" placeholder="Brief description (200 characters max)..."><?= htmlspecialchars($course['short_description'] ?? '') ?></textarea>
        ...
        <div class="col-12">
                                ...
                                <div class="rich-text-wrapper">
                                    <textarea name="description" id="description" class="form-control rte" 
                                              rows="10" placeholder="Enter detailed course description..." required><?= htmlspecialchars($course['description'] ?? '') ?></textarea>
                                </div>
        ...
        <div class="col-md-4">
                                ...
                                <input type="number" step="0.01" min="0" name="price" class="form-control" 
                                       value="<?= htmlspecialchars($course['price'] ?? '0') ?>" required>
        ...
        <div class="col-md-4">
                                ...
                                <select name="level" class="form-select">
                                    <option value="beginner" <?= ($course['level'] ?? 'all') === 'beginner' ? 'selected' : '' ?>>Beginner</option>
                                    <option value="intermediate" <?= ($course['level'] ?? 'all') === 'intermediate' ? 'selected' : '' ?>>Intermediate</option>
                                    <option value="advanced" <?= ($course['level'] ?? 'all') === 'advanced' ? 'selected' : '' ?>>Advanced</option>
                                    <option value="all" <?= ($course['level'] ?? 'all') === 'all' ? 'selected' : '' ?>>All Levels</option>
                                </select>
        </div>
        <div class="col-md-4">
                                <label class="form-label fw-bold">
        ...
        <div class="col-md-6">
                                ...
                                <input type="text" name="file_type" class="form-control" 
                                       value="<?= htmlspecialchars($course['file_type'] ?? '') ?>"
        ...
        <div class="col-md-6">
                                ...
                                <input type="text" name="file_size" class="form-control" 
                                       value="<?= htmlspecialchars($course['file_size'] ?? '') ?>"
        ...
        <div class="col-md-6">
                                ...
                                <input type="text" name="license" class="form-control" 
                                       value="<?= htmlspecialchars($course['license'] ?? 'Commercial') ?>"
                                       placeholder="Commercial, Royalty-Free, etc.">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card course-form-card">
                    ...
                    <div class="card-body">
                        <div class="alert alert-info mb-4">
                            ...
                            <strong>Media Guidelines:</strong> The promo video appears as a "Watch Trailer" button on the course details page. Course videos should be managed via Modules & Lessons.
                        </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold">
                                    <i class="fas fa-image me-1 text-primary"></i>Course Thumbnail Image
                                </label>
                                ...
                                <?php if (!empty($course['cover_image'])): ?>
                                    <div class="mt-2">
                                        ...
                                        <img src="../<?= htmlspecialchars($course['cover_image']) ?>" 
                                             class="preview-thumbnail" alt="Current thumbnail" style="max-width: 200px; border-radius: 8px;">
                                    </div>
            <?php endif; ?>
                                <div id="thumbnailPreview" class="mt-2"></div>
                                <small class="text-muted">
                                    <i class="fas fa-lightbulb me-1"></i>Recommended: 1280x720px (16:9 ratio). This appears on course listings and as the trailer thumbnail.
                                </small>
        </div>
        ...
        <div class="col-md-6">
                                <label class="form-label fw-bold">
                                    <i class="fas fa-video me-1 text-danger"></i>Course Promo Video (Trailer)
                                </label>
                                <input type="url" name="promo_video" class="form-control" 
                                       value="<?= htmlspecialchars($course['promo_video'] ?? '') ?>"
                                       placeholder="https://www.youtube.com/watch?v=... or https://drive.google.com/file/d/...">
                                <small class="text-muted">
                                    <i class="fas fa-lightbulb me-1"></i>YouTube, Vimeo, or Google Drive URL. This will be shown as a "Watch Trailer" button on the course page.
                                </small>
                                <?php if (!empty($course['promo_video'])): ?>
                                    <div class="mt-2">
                                        <a href="<?= htmlspecialchars($course['promo_video']) ?>" target="_blank" 
                                           class="btn btn-sm btn-outline-primary">
                                            <i class="fas fa-external-link-alt me-1"></i>View Current Trailer
                                        </a>
                                    </div>
            <?php endif; ?>
        </div>
        ...
        <div class="col-md-6">
                                ...
                                <?php if (!empty($course['file_path'])): ?>
                                    <div class="small mt-2">
                                        ...
                                        <a href="../<?= htmlspecialchars($course['file_path']) ?>" target="_blank" 
        ...
        <div class="col-md-6">
                                ...
                                <input type="url" name="file_url" class="form-control" 
                                       value="<?= htmlspecialchars($existingFileUrl) ?>"
        ...
        <div class="col-md-6">
                                ...
                                <input type="url" name="demo_audio_url" class="form-control" 
                                       value="<?= htmlspecialchars($existingDemoAudio) ?>"
        ...
        <div class="col-md-6">
                                ...
                                <input type="url" name="original_source" class="form-control" 
                                       value="<?= htmlspecialchars($existingOriginalSource) ?>"
                ...
                $existingModules = [];
                $modulesStmt = $pdo->prepare("SELECT * FROM modules WHERE course_id = ? ORDER BY order_number ASC, id ASC");
                $modulesStmt->execute([$course['id']]);
                $modules = $modulesStmt->fetchAll(PDO::FETCH_ASSOC);
                
                foreach ($modules as $module) {
                    $lessonsStmt = $pdo->prepare("SELECT * FROM lessons WHERE module_id = ? ORDER BY order_number ASC, id ASC");
                            ...
                            <?php if (empty($existingModules)): ?>
                                ...
                                <div class="curriculum-module mb-4 p-3 border rounded" data-module-index="0">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea name="modules[0][summary]" class="form-control form-control-sm" rows="2" 
        ...
        <div class="col-md-6">
                            ...
                            <?php else: ?>
                                ...
                                <?php foreach ($existingModules as $mIdx => $module): ?>
                                    <div class="curriculum-module mb-4 p-3 border rounded" data-module-index="<?= $mIdx ?>">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <h6 class="mb-0 fw-bold">
                                                <i class="fas fa-folder me-2 text-primary"></i>Module <?= $mIdx + 1 ?>
                                            </h6>
                                        ...
                                        <div class="row g-3 mb-3">
                                            <div class="col-md-8">
                                                <label class="form-label small fw-bold">Module Title <span class="text-danger">*</span></label>
                                                <input type="text" name="modules[<?= $mIdx ?>][title]" class="form-control form-control-sm" 
                                                       value="<?= htmlspecialchars($module['title']) ?>" required>
        </div>
        <div class="col-md-4">
                                                <label class="form-label small fw-bold">Order</label>
                                                <input type="number" name="modules[<?= $mIdx ?>][order]" class="form-control form-control-sm" 
                                                       value="<?= (int)$module['order_number'] ?>" min="1">
        </div>
            </div>
                                        
                                        <div class="mb-3">
                                            <label class="form-label small fw-bold">Module Summary (Optional)</label>
                                            <textarea name="modules[<?= $mIdx ?>][summary]" class="form-control form-control-sm rte-curriculum" rows="3"><?= htmlspecialchars($module['summary'] ?? '') ?></textarea>
                                        </div>
                                        
                                        <div class="lessons-container">
                                            <?php foreach ($module['lessons'] as $lIdx => $lesson): ?>
                                                <div class="lesson-item mb-3 p-3 bg-light rounded" data-lesson-index="<?= $lIdx ?>">
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        <h6 class="mb-0 small fw-bold">
                                                            <i class="fas fa-play-circle me-1 text-success"></i>Lesson <?= $lIdx + 1 ?>
                                                        </h6>
                                                    ...
                                                    <div class="row g-2 mb-2">
                                                        <div class="col-md-6">
                                                            <label class="form-label small">Lesson Title <span class="text-danger">*</span></label>
                                                            <input type="text" name="modules[<?= $mIdx ?>][lessons][<?= $lIdx ?>][title]" class="form-control form-control-sm" 
                                                                   value="<?= htmlspecialchars($lesson['title']) ?>" required>
        </div>
        <div class="col-md-3">
                                                            <label class="form-label small">Content Type</label>
                                                            <select name="modules[<?= $mIdx ?>][lessons][<?= $lIdx ?>][content_type]" class="form-select form-select-sm">
                                                        ...
                                                        <div class="col-md-3">
                                                            <label class="form-label small">Duration</label>
                                                            <input type="text" name="modules[<?= $mIdx ?>][lessons][<?= $lIdx ?>][duration]" class="form-control form-control-sm" 
                                                                   value="<?= htmlspecialchars($lesson['duration'] ?? '') ?>" placeholder="e.g., 10:30">
        </div>
                                                    </div>
                                                    
                                                    <div class="mb-2">
                                                        <label class="form-label small">Video URL (YouTube, Vimeo, Google Drive, or direct link)</label>
                                                        <input type="url" name="modules[<?= $mIdx ?>][lessons][<?= $lIdx ?>][video_url]" class="form-control form-control-sm" 
                                                    ...
                                                    <div class="mb-2">
                                                        <label class="form-label small">Text Content (for text lessons)</label>
                                                        <textarea name="modules[<?= $mIdx ?>][lessons][<?= $lIdx ?>][text_content]" class="form-control form-control-sm rte-curriculum" rows="4"><?= htmlspecialchars($lesson['text_content'] ?? '') ?></textarea>
                                                    </div>
                                                    
                                                    <div class="form-check">
                                                        <input type="checkbox" name="modules[<?= $mIdx ?>][lessons][<?= $lIdx ?>][is_preview]" class="form-check-input" 
                                                               id="preview_<?= $mIdx ?>_<?= $lIdx ?>" <?= !empty($lesson['is_preview']) ? 'checked' : '' ?>>
                                                        <label class="form-check-label small" for="preview_<?= $mIdx ?>_<?= $lIdx ?>">
                                                            <i class="fas fa-eye me-1"></i>Mark as Preview (free access for non-enrolled students)
                                                        </label>
                                                    </div>
                                                </div>
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-primary add-lesson" data-module-index="<?= $mIdx ?>">
                                            <i class="fas fa-plus me-1"></i>Add Lesson
                                        </button>
                                    </div>
                                <?php endforeach; ?>
        ...
        <div class="col-md-6">
                                ...
                                <div class="category-checkbox-wrapper border rounded p-3" style="max-height: 300px; overflow-y: auto;">
                                    ...
                                    foreach ($parents as $p) {
                                        ...
                                        if ($kids) {
                                            foreach ($kids as $ch) {
                                                $checked = in_array((int)$ch['id'], $existingCategoryIds, true) ? 'checked' : '';
                                                $uniqueId = 'cat_' . $ch['id'];
                                                echo '<div class="form-check ms-3">';
                                                echo '<input type="checkbox" name="category_ids[]" class="form-check-input" id="' . $uniqueId . '" value="' . (int)$ch['id'] . '" ' . $checked . '>';
                                                echo '<label class="form-check-label" for="' . $uniqueId . '">' . htmlspecialchars($ch['name']) . '</label>';
                                                echo '</div>';
                                            }
                                        } else {
                                            $checked = in_array((int)$p['id'], $existingCategoryIds, true) ? 'checked' : '';
                                            $uniqueId = 'cat_' . $p['id'];
                                            echo '<div class="form-check ms-3">';
                                            echo '<input type="checkbox" name="category_ids[]" class="form-check-input" id="' . $uniqueId . '" value="' . (int)$p['id'] . '" ' . $checked . '>';
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="publisher" class="form-control" 
                                       value="<?= htmlspecialchars($course['publisher'] ?? 'AK23STUDIOKITS') ?>"
                                ...
                                <input type="text" name="password_hint" class="form-control" 
                                       value="<?= htmlspecialchars($course['password_hint'] ?? 'ak23studiokits.com') ?>"
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="visibility" class="form-select" id="visibilitySelect">
                                <option value="draft" <?= ($course['visibility'] ?? 'draft') === 'draft' ? 'selected' : '' ?>>Draft - Save for later</option>
                                <option value="published" <?= ($course['visibility'] ?? 'draft') === 'published' ? 'selected' : '' ?>>Published - Make it live</option>
                                <option value="scheduled" <?= ($course['visibility'] ?? 'draft') === 'scheduled' ? 'selected' : '' ?>>Scheduled - Publish later</option>
                            </select>
                        </div>

                        <div class="mb-3" id="scheduleDateGroup" style="display: <?= ($course['visibility'] ?? 'draft') === 'scheduled' ? 'block' : 'none' ?>;">
                            <label class="form-label fw-bold">Schedule Publish Date</label>
                            <input type="datetime-local" name="publish_date" class="form-control" 
                                   id="publishDate" value="<?= !empty($course['created_at']) && ($course['visibility'] ?? 'draft') === 'scheduled' ? date('Y-m-d\TH:i', strtotime($course['created_at'])) : '' ?>">
                        </div>

                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="is_featured" 
                                   id="is_featured" <?= !empty($course['featured']) ? 'checked' : '' ?>>
                        ...
                        <div class="alert alert-info mb-0">
                            <small>
                                ...
                                <strong>Tip:</strong> Changes are saved immediately when you click Update Course.
                            </small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card course-form-card">
                    ...
                    <div class="card-body">
                        <div class="text-center mb-3">
                            ...
                            <h6 id="previewTitle" class="mb-1"><?= htmlspecialchars($course['title'] ?? 'Course Title') ?></h6>
                            <p class="text-muted small mb-0" id="previewPrice">TZS <?= number_format((float)($course['price'] ?? 0), 2) ?></p>
                        </div>
                        <hr>
                        <div class="small">
                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Status:</span>
                                <span id="previewStatus" class="badge bg-<?= ($course['visibility'] ?? 'draft') === 'published' ? 'success' : (($course['visibility'] ?? 'draft') === 'scheduled' ? 'warning' : 'secondary') ?>">
                                    <?= ucfirst($course['visibility'] ?? 'draft') ?>
                                </span>
                            </div>
                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Level:</span>
                                <span id="previewLevel"><?= ucfirst($course['level'] ?? 'all') ?></span>
                            </div>
                            <div class="d-flex justify-content-between">
                                <span class="text-muted">Categories:</span>
                                <span id="previewCategories" class="text-end">
                                    <?php
                                    if (!empty($existingCategoryIds)) {
                                        $catNames = [];
                                        foreach ($categories as $cat) {
                                            if (in_array((int)$cat['id'], $existingCategoryIds)) {
                                                $catNames[] = $cat['name'];
                                            }
                                        }
                                        echo htmlspecialchars(implode(', ', array_slice($catNames, 0, 2)));
                                        if (count($catNames) > 2) echo '...';
                                    } else {
                                        echo 'None selected';
                                    }
                                    ?>
  ...
  function tryInit() {
    ...
    if (shortDesc && !tinymce.get('short_description')) {
      try {
        ...
        console.error('TinyMCE init error for short_description:', e);
      }
    }
    ...
    tinymce.init({
        ...
        console.error('TinyMCE init error for description:', e);
      }
    }
  }
...
<script>
// Curriculum Management JavaScript (same as add_course.php)
(function() {
  const existingModules = <?= count($existingModules) ?>;
  let moduleIndex = existingModules;
  let lessonIndices = {};
  
  // Initialize lesson indices for existing modules
  <?php foreach ($existingModules as $mIdx => $module): ?>
    lessonIndices[<?= $mIdx ?>] = <?= count($module['lessons']) ?>;
  <?php endforeach; ?>
    ...
    if (e.target.closest('.add-lesson')) {
      ...
      setTimeout(function() {
        ...
        if (newTextArea && typeof tinymce !== 'undefined') {
          const uniqueId = newTextArea.id || 'rte_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
